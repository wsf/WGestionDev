...
Canal1 = FreeFile
Open App.Path + "\CITI\REGINFO_CV_VENTAS_CBTE.TXT" For Output As Canal1

Canal2 = FreeFile
Open App.Path + "\CITI\REGINFO_CV_VENTAS_ALICUOTAS.TXT" For Output As Canal2
...
Dim ponerenciti As Boolean
    ...
    With bTemp_Iva
        ...
        .RecordSource = "SELECT * FROM Temp_Iva ORDER BY Fecha ASC, TipoMovimiento ASC, NroComprobante"
        .Refresh
       ...
       ' vtipoDocu = getTipoDoc(.Recordset("TipoMovimiento").Value)
        ...
        vlb = vlb + fn(80, 2, "N") ' Código de documento del comprador 6
        
        vlb = vlb + fn(.Recordset("Cuit").Value, 20, "N") ' Código de documento del comp ' 7'
...
b1.Value = 0
    ...
    If Not chkTotales.Value = xtpChecked Then
        With bFacturas
            ...
            .RecordSource = "SELECT * FROM factura WHERE fecha >= '" + strfechaMySQL(dtpFecha(0)) + "' and fecha <= '" + strfechaMySQL(dtpFecha(1)) + "' AND (tipo <> 'Documento' OR Tipo Is NULL) and (Letra = 'A' or Letra = 'B')   ORDER BY Fecha ASC, tipo ASC, Ncomprobante ASC"
            
            If chkcae.Value Then
                .RecordSource = "SELECT * FROM factura WHERE (not cae = '' and not cae is null) and fecha >= '" + strfechaMySQL(dtpFecha(0)) + "' and fecha <= '" + strfechaMySQL(dtpFecha(1)) + "' AND (tipo <> 'Documento' OR Tipo Is NULL) and (Letra = 'A' or Letra = 'B')   ORDER BY Fecha ASC, tipo ASC, Ncomprobante ASC"
            End If
