...
Object = "{63BEADB1-20E1-478A-9B40-DDDAFBF3624F}#1.0#0"; "Copia de bsGradientLabel.ocx"
...
If Me.opTipoDoc(3).Value Or chkReingresarFact.Value = xtpChecked Then
    obtenerCAE = True
...
If Not obtenerCAE Then Exit Sub
        ...
        If Me.vGrabaModo = 0 Then
            ...
            txtNroInterno.Text = UltimoNroInterno2 + 1
        End If
   ...
   ' MsgBox "Prepare la Impresora ", vbInformation, "Mensaje ..."
    ...
    With frmPrincipal.FiscalHasar
        ...
        Select Case EsNulo(rsImprimirHasar.Fields("TipoIva").Value)
            ...
            Case "Iva Exento"
                .PrecioBase = False
                Call .DatosCliente(rsImprimirHasar.Fields("Nombre").Value, Replace(rsImprimirHasar.Fields("Cuit").Value, "-", ""), TIPO_CUIT, RESPONSABLE_EXENTO, Left(EsNulo(rsImprimirHasar.Fields("Direccion").Value), 50))
        ...
        Select Case EsNulo(rsImprimirHasar.Fields("Tipo").Value)
            ...
            Case "Nota C"
                .InformacionRemito(1) = "0001-" & FormatoNroFactura(vNroFacturaNotaC)
                
                If EsNulo(rsImprimirHasar.Fields("idTipoIva").Value) = "001" Then
                   ' .AbrirComprobanteNoFiscalHomologado NOTA_CREDITO_A
                    .AbrirComprobanteNoFiscalHomologado TICKET_NOTA_CREDITO_A
                Else
                   ' .AbrirComprobanteNoFiscalHomologado NOTA_CREDITO_B
                    .AbrirComprobanteNoFiscalHomologado TICKET_NOTA_CREDITO_B
                End If
            Case "Remito"
                '
        End Select
...
Private Sub CalcularTotales()
   ...
   ' vdescuento = 0
    ...
    If opTipoDoc(5).Value = True Or opTipoDoc(0).Value = True Or opTipoDoc(2).Value = True Then
          ...
          '  End If
                ...
                If Not KlexDetalle.TextMatrix(i, 21) = "S" Then
                    ...
                    If TipoDocumento = "Nota C" Then
                        Call GuardarEnStock("Devolucion", EsNulo(.Fields("Codigo").Value), strfechaMySQL(dtpFecha.Value), Val(KlexDetalle.TextMatrix(i, 5)), "Devolucion de Mercaderia", KlexDetalle.TextMatrix(i, 1), 0)
                    Else
                        If Not TipoDocumento = "Presupuesto" Then
                            If Not KlexDetalle.TextMatrix(i, 24) = "S" Then
                                Call GuardarEnStock("Remito-Nuevo", EsNulo(.Fields("Codigo").Value), strfechaMySQL(dtpFecha.Value), Val(KlexDetalle.TextMatrix(i, 5)), "Entrada de Mercaderia", KlexDetalle.TextMatrix(i, 1), 0)
                            Else
...
Function getTipoParaFE() As Integer
...
vNroComprobanteAnterior = fe.f1CompUltimoAutorizado((Val(Me.cboPuntoDeVenta.Text)), Val(vtipoFactura))
vNroComprobanteAnterior = vNroComprobanteAnterior
vultimoMensajeError = fe.UltimoMensajeError
...
+ Chr(13) + " - Ticket hora de vencimiento: " + fe.f1TicketHoraVencimiento + " " + Str(fe.f1TicketValido)
            ...
            Else
                    ...
                    Else
                                        ...
                                        If Val(vBusqueda) > 0 And Not UCase(LeerXml("Puesto")) = "KIOSCO" Then
                                            sqlArticulos = "SELECT * FROM Articulos WHERE (Codigo = '" & Trim(vBusqueda) & "')"
                                        Else
                        ...
                        With rsArticulos
                            ...
                            Else
                              ...
                              If Val(Trim(Me.txtDetalle(1).Text)) > 11111111 Then
                                    ...
                                    barticulo.RecordSource = "SELECT * FROM Articulos WHERE (CodigoBarra = '" & Trim((Val(Trim(Me.txtDetalle(1).Text)))) & "') order by pventa1 desc limit 1"
                                    barticulo.Refresh
                                Else
...
vsaldodeudor = Val(lblsaldocliente.Caption)
        ...
        If Me.rdT.Value Then
            ...
            If UCase(LeerXml("Impresora")) = UCase("Fiscal Ticket HASAR") Then Call ImprimirHasar(vremito, 0)
...
If UCase$(LeerXml("Puesto")) = "DIEGO" Then
...
If UCase(LeerXml("Puesto")) = "KIOSCO" Then
   ...
   ' Call txtCliente_KeyPress(0, 13)
     ...
     'Call MostrarCoincidencias("Clientes", Trim("GENERICO"))
...
bdetalle.RecordSource = "select * from fdetalle where remito=" + Str(Val(txtNroRemito.Text))
bdetalle.Refresh
...
If Not UCase(LeerXml("ObtieneCAE")) = UCase("SI") Then
...
If UCase(LeerXml("ObtieneCAE")) = UCase("SI") Then
...
If UCase(LeerXml("Puesto")) = UCase("Poliwheel") Or True Then ' cambiar or true
    ...
    With KlexDetalle
        ...
        If TabTipoDetalle.tab = 0 Then
           ...
           .TextMatrix(j, 4) = "[" & Trim(txtDetalle(1).Tag) & "]"   '(codigo)
            ...
            If LeerXml("PrecioConIva") = "False" Then


            Else
                        If (UCase(LeerXml("Puesto")) = "KIOSCO") And Me.opTipoDoc(0).Value And Me.cboTipoIva = "Iva Responsable Inscripto" Then
...
If Err Then GrabarLog "WCtaCte", Err.Number & " " & Err.Description, Me.Name
End Sub
'Metodo obsoleto
