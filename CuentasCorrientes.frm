...
Begin VB.Form frmCtaCteC 
   ...
   Begin XtremeSuiteControls.GroupBox GroupBox1 
      ...
      End
   End
   Begin XtremeSuiteControls.FlatEdit vctipo 
      Height          =   315
      Left            =   1620
      TabIndex        =   0
      Top             =   1650
...
Private Sub FiltrarMovimientos()
   ...
   Else
       ...
       sqlFiltro = "SELECT " + fcampos + " FROM " + vtabla + " WHERE (codigo = '" & Trim(txtCliente.Tag) & "') and (Fecha >= '" & strfechaMySQL(dtpDesde.Value) + "' and fecha <= '" & strfechaMySQL(dtpHasta.Value) + "')" + vsqlTipo + " order by fecha," + vid
    vcriterio = vcriterio + "Comentario: " + vcomentario + " - " + vsqlTipo
