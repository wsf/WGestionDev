...
On Error Resume Next
    ...
    With Mantenimiento.rsDiario
        ...
        .Source = "SHAPE {SELECT * FROM Asientos WHERE 1=1 " + vsql + " order by fecha} AS Diario APPEND ({SELECT * FROM AsientosDetalle INNER JOIN Cuentas ON AsientosDetalle.CodigoCuenta = Cuentas.CodigoCuenta where debe+haber > 0}  AS AsientosDetalle RELATE 'Numero' TO 'Numero') AS AsientosDetalle"
